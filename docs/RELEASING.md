...
`gotth-notify` versions independently under Semantic Versioning. A version number
...
No release may be tagged until required verification is complete. The
repository uses the MIT license. Reserved placeholders are not tagged at all.
...
5. Verify exact head and tag parity at
   `github.com/gotthboard/gotth-notify` through the existing one-way push
   mirror.
